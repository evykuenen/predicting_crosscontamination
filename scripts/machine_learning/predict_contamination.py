...
Author: Evy Kuenen
Date: 15-5-2025
Functionality:  This script predicts the labels of the inputted prediction file without labels and outputs html file
                with prediction.
Usage:
    ...
    Calling script: "python predict_contamination.py"
"""
...
def process_before_predict(df):
    ...
    """
    used_columns = ['ani', "correlation", "fragmentation", "average contig length", "contig_count"]
...
def predict(df, model_path):
    ...
    result_df = pd.DataFrame({
        ...
        "contig_count": df["contig_count"].values,
    })
...
def visualize_prediction_html(prediction_df, template_path, output_path):
    ...
    hidden_columns = {"prob_class_1"}

    # HTML table
    table_html = "<table border='1'><thead><tr>"
    table_html += "".join(
        [f"<th>{column_renames.get(col, col)}</th>" for col in prediction_df.columns if col not in hidden_columns]
    )
    ...
    for _, row in prediction_df.iterrows():
        ...
        for col, val in row.items():
            if col in hidden_columns:
                continue
            if col == "prediction":
                val = "contaminated" if val == 1 else "clean"
    ...
    print(f"HTML popup generated: '{output_path}'")
...
def main():
    # input
    prediction_csv = "..\\..\\output_from_scripts\\test_train_data_ml\\prediction.csv"

    #output
    template_path = "..\\..\\output_from_scripts\\html_prediction\\popup_template.html"
    output_path = "..\\..\\output_from_scripts\\html_prediction\\contamination_popup.html"

    model_path = "models\\ensemble_2xaugment_mcc0.54.joblib"
    prediction_dataframe = pd.read_csv(prediction_csv)
    prediction_df = predict(prediction_dataframe, model_path)
