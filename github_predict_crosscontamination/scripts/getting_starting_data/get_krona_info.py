...
def read_krona_files(krona_files, sample_host, sample_host_logboeken):
    ...
    for file in krona_files:
        ...
        id_pattern = r"(\d{6,}-\d{3,}-\d{3,})"  # Matches only sample name, genome scan
...
def batch_match(id_match, filename, sample_host, sample_host_logboeken, file):
    ...
    Get organism name and make sample, organism name, virus with contigs line.
...
def find_host_if_unknown(path_lims_to_sample, path_extra_hosts):
    """
    get host name from bcf files.
    :param path_lims_to_sample: path to file with lims and sample translation table.
    :param path_extra_hosts: path to bcf file with extra host names.
    :return sample_host_logboeken: dictionary with sample and host.
    """
...
def main():
    # input
    path_krona_dir = "..\\..\\test_data\\data\\Krona_output_archive_and_data\\*"
    path_lims_to_sample = "..\\..\\output_from_scripts\\sample_ids_contaminated_samples\\sample_to_lims.csv"
    path_excel_dir = "..\\..\\test_data\\data\\label_data_virology\\Virologie\\Virologie"
    path_extra_hosts = "..\\..\\test_data\\data\\Logboeken officiele documenten"
    directory_name = "..\\..\\output_from_scripts\\getting_sample_host_virus_from_krona_files"

    # output
    path_normalized_data = "..\\..\\output_from_scripts\\getting_sample_host_virus_from_krona_files\\krona_data.csv"
    path_hosts = "..\\..\\output_from_scripts\\getting_sample_host_virus_from_krona_files\\hosts_in_krona_files.txt"
